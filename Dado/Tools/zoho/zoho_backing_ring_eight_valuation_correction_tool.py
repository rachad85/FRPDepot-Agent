...
def acquire_lock(plan: dict[str, Any]) -> Path:
    ...
    path = lock_path(plan)
    record = {"plan_sha256": plan["sha256"], "action": ACTION, "locked_utc": utc_now().isoformat(), "state": "commit_started"}
    try:
        descriptor = os.open(path, os.O_WRONLY | os.O_CREAT | os.O_EXCL, 0o600)
    except FileExistsError as exc:
        raise ValueCorrectionError("REFUSED: this plan is already commit-locked; no retry is allowed.") from exc
...
def request_result(request: Request) -> dict[str, Any]:
    ...
    try:
        ...
        raise ValueCorrectionError(f"Zoho write failed with HTTP {exc.code}: {detail}; plan is permanently locked.") from exc
    except (URLError, TimeoutError, OSError, json.JSONDecodeError) as exc:
        raise ValueCorrectionError(f"Zoho write result is indeterminate: {exc}; plan is permanently locked.") from exc
    if result.get("code") not in (None, 0):
        raise ValueCorrectionError(f"Zoho write failed: {result.get('message') or result.get('code')}; plan is permanently locked.")
    return result
...
def command_commit(args: argparse.Namespace) -> None:
    if args.approval != APPROVAL_WORD:
        raise ValueCorrectionError("REFUSED: approval must be exactly unpadded uppercase APPROVED.")
    ...
    if lock_path(plan).exists():
        raise ValueCorrectionError("REFUSED: this plan is already commit-locked; no retry is allowed.")
    ...
    try:
        ...
        update_lock(lock, "indeterminate", {"error": str(exc), "inventory_adjustment_id": adjustment_id or None, "no_retry": True})
        raise
...
def build_parser() -> argparse.ArgumentParser:
    ...
    commit.add_argument("--plan", required=True); commit.add_argument("--approval", required=True)
