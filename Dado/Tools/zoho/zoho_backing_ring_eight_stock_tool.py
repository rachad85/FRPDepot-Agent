...
def acquire_lock(plan: dict[str, Any]) -> Path:
    ...
    try:
        ...
        raise EightBackingRingStockError("REFUSED: this plan is already commit-locked; no retry is allowed.") from exc
...
def request_result(request: Request) -> dict[str, Any]:
    ...
    try:
        ...
        raise EightBackingRingStockError(f"Zoho write failed with HTTP {exc.code}: {detail}; plan is permanently locked.") from exc
    except (URLError, TimeoutError, OSError, json.JSONDecodeError) as exc:
        raise EightBackingRingStockError(f"Zoho write result is indeterminate: {exc}; plan is permanently locked.") from exc
    if result.get("code") not in (None, 0):
        raise EightBackingRingStockError(f"Zoho write failed: {result.get('message') or result.get('code')}; plan is permanently locked.")
    return result
...
def command_commit(args: argparse.Namespace) -> None:
    if args.approval != APPROVAL_WORD:
        raise EightBackingRingStockError("REFUSED: approval must be exactly unpadded uppercase APPROVED.")
    ...
    if lock_path(plan).exists():
        raise EightBackingRingStockError("REFUSED: this plan is already commit-locked; no retry is allowed.")
