#!/usr/bin/env python
"""FRP Depot Loans Spreadsheet Approved Repayment Tool.
...
native spreadsheet My Drive/My Files/Rachad/Bussiness Folder/Loans, tab CCIVS,
table A4:B60. It cannot create, delete, rename, move, share, clear, batch, or
otherwise restructure anything: no other remote write path exists in the tree.
The only operation currently commissioned is filling the next existing blank repayment row after a
24-hour, full-SHA-256, single-use staged approval.
...
def build_plan(item: dict[str, Any], identity: dict[str, Any], grid: list[list[str]],
               entry: dict[str, Any], source: str, *, created: datetime | None = None) -> dict[str, Any]:
...
def load_plan(path: Path) -> dict[str, Any]:
    ...
    if (
        ...
        or file_info.get("tab") != SHEET_TITLE
        or file_info.get("sheet_id") != SHEET_ID
    ...
    if rebuilt != entry or not FIRST_ROW <= row <= LAST_ROW:
...
def recheck_live(drive, sheets, plan: dict[str, Any]) -> list[list[str]]:
    ...
    if content_sha256(grid) != plan["file"]["content_sha256"]:
        raise LoansError("The live CCIVS values changed after review. Stage a new plan.")
    entry = plan["entry"]
    row = int(entry["row"])
    if grid[row - 1][0].strip() or grid[row - 1][1].strip():
        raise LoansError(f"CCIVS row {row} is no longer blank. Stage a new plan.")
    state = table_state(grid)
    if int(state["next_row"]) != row:
        raise LoansError("The CCIVS next free row moved after review. Stage a new plan.")
    if format(Decimal(state["current_balance"]), "f") != str(entry["current_balance_cad"]):
        raise LoansError("The CCIVS balance changed after review. Stage a new plan.")
    return grid
...
def _check_append_response(response: Any, entry: dict[str, Any]) -> None:
    ...
    if normalized != f"{SHEET_TITLE}!A{row}:B{row}":
        raise LoansError(
            f"Sheets wrote {actual or 'an unreported range'} instead of the planned "
            f"{SHEET_TITLE}!A{row}:B{row}; reconciliation required."
        )
    if (
        int(updates.get("updatedRows") or 0) != 1
        or int(updates.get("updatedColumns") or 0) != 2
        or int(updates.get("updatedCells") or 0) != 2
    ):
...
def command_commit(args: argparse.Namespace) -> None:
    ...
    backup = BACKUP_DIR / (utc_now().strftime("%Y%m%dT%H%M%SZ_") + digest[:16] + "_CCIVS_A1_B60.json")
    ...
    try:
        ...
        if recheck_live(drive, sheets, plan) != before:
            raise LoansError("The live CCIVS values changed immediately before the write.")
        ...
        append_receipt(
            f"loans_ccivs_payment_{status}_no_retry",
            f"plan={plan_path}; sha256={digest}; backup={backup}",
        )
        if write_attempted:
            raise LoansError(
                "The CCIVS append failed or could not be verified. The plan is locked and will "
                "not retry. Reconciliation required: compare the live CCIVS tab against the "
    ...
    append_receipt(
        "loans_ccivs_payment_committed_verified",
