...
- **FRP Depot Windows/Hermes:** the inherited `PYTHONPATH` points at the Hermes venv and can shadow the uv-tool environment's compiled `lxml`. For every terminal invocation on this profile, use `PYTHONPATH= ddgs ...`, including `PYTHONPATH= ddgs --help`. A bare `ddgs` command is not a valid readiness check here.
