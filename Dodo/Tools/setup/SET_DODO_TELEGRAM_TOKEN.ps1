# SET_DODO_TELEGRAM_TOKEN.ps1 — store Dodo's Telegram bot token on THIS PC only.
# Saves to a local vault AND into Dodo's Hermes profile .env. The token is
# never uploaded to GitHub, never shown on screen, never pasted in chat.

$ErrorActionPreference = "Stop"

$vaultDir = Join-Path $env:LOCALAPPDATA "FRPDepot-Dodo"
New-Item -ItemType Directory -Force -Path $vaultDir | Out-Null
$vault = Join-Path $vaultDir "telegram.env"
$profileEnv = Join-Path $env:LOCALAPPDATA "hermes\profiles\dodo\.env"

Write-Host "==============================================================="
Write-Host "  Set Dodo's Telegram bot token  (THIS PC only)"
Write-Host "==============================================================="
Write-Host ""
Write-Host "Where to get the token: open Telegram, message @BotFather, send"
Write-Host "  /mybots  ->  pick Dodo's bot  ->  API Token."
Write-Host ""
Write-Host "It is saved only on this PC. It is never uploaded and never shown."
Write-Host ""

$sec = Read-Host "Paste Dodo's Telegram BOT TOKEN, then press Enter" -AsSecureString
...
if (-not (Test-Path $profileEnv)) {
  Write-Host "Dodo's profile was not found on this PC. Nothing was saved."
  exit 1
}
...
# 2) Inject into Dodo's profile .env (replace-or-append, ASCII only)
...
Write-Host ""
Write-Host "Saved. Only your Telegram account (891365639) can talk to Dodo."
Write-Host ""
Write-Host "NEXT STEP: double-click START_DODO.bat (or STOP then START if she"
